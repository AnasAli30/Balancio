...
<tr className='tablerow'>
            <td className='first'>
                ...
                {pro.block_hash.slice(0,6) + "..." + pro.block_hash.slice(-4)}
                </div>
            </td>
            <td className='second'>
            <img 
  ...
  <div>
    ...
    <p>{pro.from_address.slice(0,6) + "..." + pro.from_address.slice(-4)}</p>
            ...
            {pro.native_transfers.length?<td className='third'>
                ...
                <p>{pro.native_transfers[0].direction=="receive"?"+":"-"} { parseFloat(
                     ethers.utils.formatUnits( pro.native_transfers[0].value, 18)).toFixed(2)} {pro.native_transfers[0].token_symbol} {
                    parseFloat(
                     ethers.utils.formatUnits( pro.native_transfers[0].value_formatted, 18)).toFixed(2)
                   
                    }$</p>
            </td>:
            <td className='third'>
            <img src={pro.to_address_entity_logo} alt="" />
            <p>+20 SAPU 0.01</p>
        </td>
            }
            </tr>
